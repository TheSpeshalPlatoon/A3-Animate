...
player addEventHandler ["InventoryOpened", {[playa, 0.5, "tsp_animate\snd\inventory_open.ogg", 5, 20, 2] call tsp_fnc_animate_effect}];
player addEventHandler ["InventoryClosed", {[playa, 0.5, "tsp_animate\snd\inventory_close.ogg", 5, 20, 2] call tsp_fnc_animate_effect}];
player addEventHandler ["Take", {if (!isNull (findDisplay 602)) then {[playa, 0.5, "tsp_animate\snd\take.ogg", 5, 20, 3] call tsp_fnc_animate_effect}}];
player addEventHandler ["Put", {if (!isNull (findDisplay 602)) then {[playa, 0.5, "tsp_animate\snd\put.ogg", 5, 20, 3] call tsp_fnc_animate_effect}}];
...
addUserActionEventHandler ["ShowMap", "Activate", {[] spawn {sleep 0.01; if (tsp_cba_animate_map && visibleMap && isNull findDisplay 312) then {
    ...
    [playa, 1, "tsp_animate\snd\map_open.ogg", 0.7, 20, 1] call tsp_fnc_animate_effect; waitUntil {!visibleMap}; [playa, 1, "tsp_animate\snd\map_close.ogg", 0.7, 20, 1] call tsp_fnc_animate_effect;
}}}];
